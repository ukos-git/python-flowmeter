...
class MKFlowMessage():
    ...
    def analyseMessage(self):
        ...
        try:
            self.parseMessage1()
            # analyse message and extract.Data
            ...
            self.Invalid.add('MKFlowMessage:analyseMessageType:\tError while extracting data.')
            if len(self.data)<3:
    ...
    class MKFlowProcess(MKFlowProcess):
        class MKFlowParameter(MKFlowParameter):
            def analyse(self):
                ...
                self.setLength(3)
                if not self.check:
                    info = 'MKFlowRequest: byte1 and byte3 do not contain the same information. byte1 is index, byte3 is FbNr'
                    #raise ValueError('MKFlowRequest: byte1 and byte3 do not contain the same information. byte1 is index w chaining, byte3 is FbNr')
            def analyseData(self):
                try:
                    if len(self.data)<3:
